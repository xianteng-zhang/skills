...
个人可复用技能合集（Claude Code / DSH 风格）。每个技能是独立目录，复制到对应工具的 skills 路径即可生效。

技能不只是「提示词模板」——这里的每个技能都带**判据、证据纪律和输出契约**，目标是让判断过程可复现、可对质，而不是让模型自由发挥。

## 定位与收录范围

这是一个**持续生长**的合集，主要收录两类技能：

- **Agent 工程**：判断、审查、设计与落地 Agent 系统时反复要用的流程与标准；
- **研发提效**：日常开发、写作、排障中值得固化成流程的重复动作。

收录标准（四条，缺一条说明它还不该进这个仓库）：

1. **可判定** —— 有明确的判据或评分口径，不靠"我觉得"；
2. **可验证** —— 结论能追溯到实测证据（命令输出、文件路径、计数）；
3. **有产出** —— 最终交付一个具体结果（判定、报告、改写后的文本），不是聊天；
4. **可迁移** —— 换个项目仍然成立，不是一次性的现场笔记。
